...
export function setIncomePlanningPlanningPopupView(view: "year" | "stamp"): void {
  ...
  incomePlanningUiState.planningPopup = { ...incomePlanningUiState.planningPopup, view };
  renderIncomePlanningPlanningPopup();
}
  ...
  root.innerHTML = `
    <div class="income-planning-dialog-backdrop" role="presentation">
      <div class="income-planning-dialog income-planning-planning-popup" role="dialog" aria-modal="true" aria-label="Jahresplanung und Stempelplaner">
        <div class="income-tax-dialog-head income-planning-popup-head">
          <div>
            <strong>${popup.view === "year" ? "Jahresplanung" : "Stempelplaner"}</strong>
            <span>${popup.view === "year" ? "Wochenszenarien fuer das Jahr planen" : "Einmalige Stempel im Monatsraster planen"}</span>
          </div>
          <div class="income-planning-popup-tabs" role="tablist" aria-label="Planungsansicht">
            ${planningPopupTab("year", "Jahresplanung", popup.view)}
            ${planningPopupTab("stamp", "Stempelplaner", popup.view)}
            <button class="income-planning-header-icon-button" type="button" data-action="income-planning-close-planning-popup" aria-label="Popup schliessen" title="Schliessen">x</button>
          </div>
        </div>
  ...
  return `
    <section class="income-planning-year-view" aria-label="Jahresplanung">
      ...
      <div class="income-planning-year-week-list">
        ${weeks.map((week) => renderIncomePlanningYearWeekRow(week, scenarios)).join("")}
      </div>
    </section>
  `;
}
...
function renderIncomePlanningYearWeekRow(
  ...
  return `
    <article class="income-planning-year-week-row" style="--income-planning-scenario-color:${escapeHtml(scenario.color)};">
      <div class="income-planning-year-week-key">
      ...
      <label class="field compact">
        <span>Szenario</span>
        <select data-income-planning-year-week-scenario="${escapeHtml(week.weekStartDate)}">
          ${scenarios.map((option) => `
            <option value="${escapeHtml(option.id)}" ${option.id === scenarioId ? "selected" : ""}>${escapeHtml(option.label)}</option>
          `).join("")}
        </select>
      </label>
    </article>
