...
function renderStatutoryPensionCalculations(birthYear: number): void {
  ...
  host.innerHTML = statutoryPensionHtml(model);
}

function statutoryPensionHtml(model: StatutoryPensionModel): string {
  const settings = state.statutoryPension;
  ...
  return `
    <article class="statutory-pension-scenario">
      <div class="statutory-pension-scenario-head">
        ...
        <span>${escapeHtml(money(scenario.projectedMonthlyPension))} brutto/Monat</span>
      </div>
...
function statutoryPensionScenarioOverlayChart(model: StatutoryPensionModel): string {
  ...
  const maxMonthlyPension = Math.max(1, ...model.scenarios.map((scenario) => scenario.projectedMonthlyPension));
  ...
  const barHtml = visualOrder
    .map((id) => {
      ...
      if (!scenario) return "";
      const width = Math.max(8, (scenario.projectedMonthlyPension / maxMonthlyPension) * 100);
      return `
        <div class="statutory-pension-overlay-bar ${id}" style="width: ${width.toFixed(2)}%;">
          <span>${escapeHtml(scenario.label)}</span>
          <strong>${escapeHtml(money(scenario.projectedMonthlyPension))}</strong>
        </div>
      `;
    })
    .join("");
    ...
    .map((id) => {
      ...
      return `
        <div class="statutory-pension-overlay-legend-card ${id}">
          <strong>${escapeHtml(scenario.label)}</strong>
          <span>Monatsrente: ${escapeHtml(money(scenario.projectedMonthlyPension))}</span>
  ...
  return `
    <div class="statutory-pension-overlay">
      <div>
        ...
        <p>Die Balken liegen uebereinander: Pessimistisch vorne, Basis darunter, Optimistisch als groesster Hintergrund.</p>
      </div>
      <div class="statutory-pension-overlay-chart" role="img" aria-label="Ueberlagerte Monatsrenten der drei Rentenszenarien">
...
function statutoryPensionNumberField(
  field: keyof AppState["statutoryPension"],
  label: string,
  value: number,
  step: number
): string {
...
function statutoryPensionScenarioRangeField(
  ...
  field: keyof AppState["statutoryPension"]["scenarios"][StatutoryPensionScenarioId],
  label: string,
  value: number,
  step: number,
  min: number,
