...
import { configureBusinessCanvasHost, renderBusinessCanvas } from "../features/self-employment/business-canvas";
...
export async function startAppController(context: AppContext, features: readonly FeatureModule[]): Promise<void> {
  ...
  configureBusinessCanvasHost({
    ...
    clearGanttEditorForDeletedNodes: (nodeIds) => {
      if (selfEmploymentGanttEditor?.type === "card" && nodeIds.has(selfEmploymentGanttEditor.cardId)) {
...
function bindEvents(features: readonly FeatureModule[]): void {
  bindAppEvents(appContext, features, {
    ...
    onPointerDown: (_event, _context) => {
      startIncomePlanningCalendarDrag(_event);
    },
    onWindowPointerMove: (_event, _context) => {
      moveIncomePlanningCalendarDrag(_event);
    },
    onWindowPointerUp: (_event, _context) => {
      finishIncomePlanningCalendarDrag(_event);
    },
